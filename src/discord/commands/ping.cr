class Ping
  def call(payload, ctx)
    ...
    latency = Discord::EmbedField.new("Latency", "#{(Time.utc_now - payload.timestamp).total_milliseconds} ms", true)
    processing_time = Discord::EmbedField.new("Processed in", elapsed_text(Time.utc_now - ctx[Command].time), true)
    ...
    roundtrip = Discord::EmbedField.new("Roundtrip", "#{(Time.utc_now - payload.timestamp).total_milliseconds} ms", true)
